...
class MiningAgent(base_agent.BaseAgent):
	...
	actions = ("do_nothing",
			...
			"balance_workers")
	def __init__(self):
		super(MiningAgent, self).__init__()
		
		self.reset()

	def reset(self):
		super(MiningAgent, self).reset()
		...
		self.units = {}
		self.positions={
			units.Terran.SupplyDepot: [(32, 44), (33, 29), (36, 43), (29, 43), (36, 29), (30, 29)],
	...
	def step(self, obs):
		...
		if obs.first():
			...
			self.command_centers.append(command_center)
...
class MiningRandomAgent(MiningAgent):
	...
	def step(self, obs):
		super(MiningRandomAgent, self).step(obs)
...
class MiningDRLAgent(MiningAgent):

	def __init__(self, H:Hyperparams=None, *args, **kwargs):
		super(MiningAgent, self).__init__()
		self.H = H or Hyperparams()
		# TODO: Do not hardcode following variables
		n_observations = 17 # form counting self.get_state return
		n_actions = 7 # do_nothing, harvest_minerals, build_supply_depot, build_barracks, train_marine, attack
	...
	def reset(self):
		super(MiningDRLAgent, self).reset()
		self.new_game()
	...
	def step(self, obs):
		super(MiningDRLAgent, self).step(obs)
		self.n_steps += 1
...
class MiningJustFarmAgent(MiningAgent):
	def step(self, obs):
		super(MiningJustFarmAgent, self).step(obs)
		return self.balance_scv_cc(obs)
		return getattr(self, "harvest_minerals")(obs)
		return getattr(self, "do_nothing")(obs)


class DebuggingAgent(base_agent.BaseAgent):
	def __init__(self, seconds_to_save=10):
		super(DebuggingAgent, self).__init__()
		self.seconds_to_save = seconds_to_save
		self.init = time.time()
		self.counter = 0
		self.saved = 0

	def save_obs(self, obs):
		savedir = "./saves/observations"
		if not os.path.isdir(savedir):
			os.makedirs(savedir)
		filename = f"obs_{self.saved}.pkl"
		file = os.path.join(savedir, filename)
		with open(file, 'wb') as f:  # open a text file
			pickle.dump(obs, f) # serialize the list

	def step(self, obs):
		super(DebuggingAgent, self).step(obs)
		pass_sec = int(time.time() - self.init)
		if pass_sec % 10 == 0 and self.saved != pass_sec:
			self.saved = pass_sec
			self.save_obs(obs)
		# return getattr(self, "harvest_minerals")(obs)
		return actions.RAW_FUNCTIONS.no_op()
...
class MiningScriptedAgent(MiningAgent):
	def __init__(self, H:Hyperparams=None, *args, **kwargs):
		super(MiningAgent, self).__init__()
		self.H = H or Hyperparams()
		# TODO: Do not hardcode following variables
