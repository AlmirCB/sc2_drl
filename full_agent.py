...
class FullAgent(base_agent.BaseAgent):
	...
	def attack(self, obs):
		...
		if len(marines) > 0:
			attack_xy = (38, 44) if self.base_top else (19, 23)
	...
	def attack2(self,obs):
		marines = self.get_my_units_by_type(obs, units.Terran.Marine)
		if len(marines) > 0:
			attack_xy = (17, 48) if self.base_top else (41, 21)
			distances = self.get_distances(marines, attack_xy)
			marine = marines[np.argmax(distances)]
			x_offset = random.randint(-4, 4)
			y_offset = random.randint(-4, 4)
...
class FullDRLAgent(FullAgent):

	def __init__(self, H:Hyperparams=None, *args, **kwargs):
		super(FullAgent, self).__init__()
		self.H = H or Hyperparams()
		# TODO: Do not hardcode following variables
		n_observations = 33 # form counting self.get_state return
		n_actions = len(self.actions)
		self.playing = kwargs.pop('playing', False)
		self.name =  kwargs.pop('name', "Full_DRL")
		self.description = kwargs.pop('description', ", ".join(self.actions))
		self.execution_id = datetime.strftime(
			datetime.now(),
			"%y%m%d%H%M%S")
		self.execution_folder = os.path.join(
	...
	def get_state(self, obs):
		...
		have_cc_1 = len(completed_command_centers) > cc1_index
		have_cc_2 = len(completed_command_centers) > cc2_index

		queued_scv_1 = (completed_command_centers[cc1_index].order_length
											if have_cc_1 else 0)
		queued_scv_2 = (completed_command_centers[cc2_index].order_length
											if have_cc_2 else 0)
		workers_cc_1 = (completed_command_centers[cc1_index].assigned_harvesters
											if have_cc_1 else 0)

		workers_cc_2 = (completed_command_centers[cc2_index].assigned_harvesters
											if have_cc_2 else 0)
	...
	def step(self, obs):
		super(FullDRLAgent, self).step(obs)
		self.n_steps += 1
